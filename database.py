...
def upsert_candidate(name: str, school: str = "", major: str = "",
                     ...
                     cert_no: str = "", reviewed: int = 0,
                     fail_reasons: str = "", **kwargs) -> int:
    """插入或更新候选人。有证书编号按编号匹配，无则按姓名+学校"""
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("ALTER TABLE candidates ADD COLUMN cert_no TEXT DEFAULT ''")
        conn.commit()
    except: pass
    ...
    # 1. 证书编号匹配 (一对一唯一)
    if cert_no:
        c.execute("SELECT id FROM candidates WHERE cert_no = ?", (cert_no,))
        row = c.fetchone()
    else:
        row = None
    ...
    if row:
        c.execute("""UPDATE candidates SET name=?, school=?, major=?, education_level=?,
            degree_cert_status=?, grad_cert_status=?, final_status=?,
            cert_no=?, reviewed=?, fail_reasons=?, updated_at=CURRENT_TIMESTAMP
            WHERE id=?""", (name, school, major, education_level, degree_status,
            grad_status, final_status, cert_no, reviewed, fail_reasons, row[0]))
        cid = row[0]
    else:
        c.execute("""INSERT INTO candidates (name, school, major, education_level,
            degree_cert_status, grad_cert_status, final_status, cert_no, reviewed, fail_reasons)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (name, school, major, education_level, degree_status, grad_status,
             final_status, cert_no, reviewed, fail_reasons))
        cid = c.lastrowid
